      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <HeroVideoCard
              video={heroVideo}
              onPlay={() => openVideo(heroVideo.id)}
              tapToWatch={tVideo("tapToWatch")}
              shortsLabel={tVideo("shorts")}
            />
          </motion.div>
          ...
          {secondaryVideo && (
            ...
            >
              <SecondaryVideoCard
                ...
                onPlay={() => openVideo(secondaryVideo.id)}
                watchExperts={tVideo("watchExperts")}
                watchNow={tVideo("watchNow")}
                shortsLabel={tVideo("shorts")}
              />
            </motion.div>
          )}
...
function HeroVideoCard({ video, onPlay, tapToWatch, shortsLabel }: HeroVideoCardProps) {
    ...
    >
      ...
      <div className="relative aspect-video md:aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 md:top-6 md:left-6 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 backdrop-blur-sm">
          ...
          <span className="text-white text-sm font-medium">{shortsLabel}</span>
        </div>
...
interface SecondaryVideoCardProps {
  video: VideoShort;
  onPlay: () => void;
  watchExperts: string;
  watchNow: string;
...
function SecondaryVideoCard({ video, onPlay, watchExperts, watchNow, shortsLabel }: SecondaryVideoCardProps) {
  const [imageError, setImageError] = useState(false);
      ...
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="relative w-full sm:w-48 md:w-64 aspect-video sm:aspect-square overflow-hidden flex-shrink-0">
          ...
          <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-full bg-black/60 backdrop-blur-sm">
            ...
            <span className="text-white text-xs font-medium">{shortsLabel}</span>
          </div>
        </div>
